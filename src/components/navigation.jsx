      ...
      <ul className={cn("flex items-center gap-6 h-16 px-4", className)}>
        {navigationMenu.map(({ text = "", link = "/", subMenus }, index) => (
          <li key={index} className="group/link">
            ...
            {subMenus && (
              <ul className="flex flex-col absolute top-full bg-white border rounded-xl py-4 -mt-3 -ml-4 opacity-0 invisible group-hover/link:opacity-100 group-hover/link:visible group-hover/link:translate-y-2">
